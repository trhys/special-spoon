...
namespace Spoon
{
    AnimationSystem::FadeAnimation(sf::Time tick, FadeComp& fadecomp, ColorComp& colorcomp)
    {
        ...
        {
            colorcomp->m_Alpha = 0.0f + (255.0f - 0.0f) * progress;
        }
        else if(fadecomp->isFadingOut)
        {
            colorcomp->m_Alpha = 255.0f + (0.0f - 255.0f) * progress;
        }

        if(progress >= 1.0f) { fadecomp->ToggleActive(); }
    }

    AnimationSystem::BlinkAnimation(sf::Time tick, BlinkComp& blinkcomp, ColorComp& colorcomp)
    {
    ...
    AnimationSystem::Run(sf::Time tick, Scene& scene)
    {
